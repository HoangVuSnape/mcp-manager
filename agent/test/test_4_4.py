...
Chú ý:
- Khi tool có tham số `userId`, cần gọi trước `get_list_employee` để lấy ID, rồi mới truyền ID vào tool tiếp theo. Hiện thị ra ID của nhân viên trước rồi mới trả lời
- Không tự bịa ra kết quả. Nếu không gọi được tool hoặc tool trả về lỗi, cần trả lời rằng không gọi được.

"""
# - mắc định default monthYear: 2025-04 
async def run(mcp_server: MCPServer):
    ...
    question = "Bạn Vonurau Mani còn bao nhiêu ngày phép trong năm 2025?"
    ...
    question = "Tổng số giờ nghỉ phép trong tháng của bạn Vonurau Mani"
    message = f"prompt {prompt} \n Question: {question} \n Trả lời: "
    print(f"\n\nRunning: {question}")
    result = await Runner.run(starting_agent=agent, input=message)
    print(result.final_output)
