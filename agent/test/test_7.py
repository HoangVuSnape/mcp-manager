...
async def run(mcp_server: MCPServer):
    ...
    # question = "Có những tool nào trong mcp server này? và viết tên và description của chúng"
    ...
    question = "Tổng số giờ/ngày làm của bạn Hoòa Nguyễn là bao nhiêu?"
    message = f"prompt {prompt} \n Question: {question} \n Trả lời: "
    print(f"\n\nRunning: {question}")
    result = await Runner.run(starting_agent=agent, input=message)
    print(result.final_output)
    ...
    print("=====================================")
    question = "Còn bao nhiêu bạn trong tháng này chưa confirm timesheet? "
    message = f"prompt {prompt} \n Question: {question} \n Trả lời: "
    print(f"\n\nRunning: {question}")
    result = await Runner.run(starting_agent=agent, input=message)
    print(result.final_output)
    ...
    print("=====================================")
    question = "Mình đã hỏi bạn những câu gì rồi "
    message = f"prompt {prompt} \n Question: {question} \n Trả lời: "
    print(f"\n\nRunning: {question}")
    result = await Runner.run(starting_agent=agent, input=message)
    print(result.final_output)
